...
module.exports = function plugin(options) {
	...
	var blurSmall = options.blurSmall || false;
	...
	return function (files, metalsmith, done){
					...
					lwip.open(files[imgPath].contents, format, function(err,image){
						...
						(function(width,height,outFile,originalWidth,options){
							...
							if(width<originalWidth){
								var i = image.batch()
								.resize(width,height);
								if(blurSmall && minimatch(imgPath,blurSmall.pattern) && width<=blurSmall.width){
									i.blur(blurSmall.amount)
								}
								i.toBuffer(format,options,function(err, buffer){
									if(err) { console.error(err); return; }
		...
		var addMarkup = function($,selector,background){
			...
			$(selector).each(function(){
				var img = $(this);
				if(background && !img.css('background-image')) return;
